...
## 5. 第 7 批 · worktree 隔离（一任务一 worktree，不做容器）

### 5.1 范围拍板（沿用 2026-09-14 拍板）
...
| 1 | 侧栏日分组 | ✅ 已落地 | T25 |
| 2 | 审批阻塞提示 | ✅ 已落地 | T26 |
| 3 | 快速捕获（Alt+X） | 待对齐确认 | T27 |
| 4 | 手动备份 | 待对齐确认 | T28 |
| 5 | worktree 隔离 | 待对齐确认 | T29 |

建议实施序：**2 → 4 → 3 → 5**（2 最小且救并发死锁；4 半天；3 需要新窗口+热键基建；5 独立性强放最后）。每项独立 commit，全量冒烟 ALL PASS 后推进下一项。
