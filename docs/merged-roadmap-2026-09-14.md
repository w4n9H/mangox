...
> 拍板记录（2026-09-15）：**§1.3「权限模式选择器」改版为「模式选择器」**——三档工具能力预设（极简/常规/完整含扩展），审批开关独立照旧；key 存 Keychain（遗留项关闭）；模型清单 `/models` 自动拉取 + 预设兜底。第 4 批详细设计见 **`docs/P7-functional-design.md`**。
...
> **2026-09-15 改版**：按用户拍板，本节设计改为**模式选择器**（三档工具能力预设：极简/常规/完整含扩展），审批开关独立保留、不做分级。详见 `docs/P7-functional-design.md` §2。以下原方案仅存档。
