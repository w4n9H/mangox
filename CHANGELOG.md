...
- **模型选择控件（`ModelPicker`）**：一颗药丸打开单面板 —— 上半模型清单、下半一根**力量滑轨**（20px 粗胶囊轨 + 把手 26px + 按停靠点对齐的刻度点）。**值进值出**：只吃 `ModelChoice`、只吐 `onPick`，不读全局 store、不写任何状态 ⇒ 同一个控件既能驱动 composer 的「全局期望」，也能驱动定时任务 / 邮箱哨兵的「仅本任务」。**换模型后的档位收敛（clamp）内聚在控件里**，宿主无须知道该模型支持哪些档
- **滑轨的「越高越紫」**：床 = 满宽渐变 @0.28 透明度、已达成段 = **同一渐变 `mask` 裁到把手位置** ⇒ 档位越高露得越多、末端越紫。配色**亮暗共用一套 蓝→紫→洋红**（`#4E62D2 → #A054E8 → #E05CC0`）—— 色相环上 227°→306° 刚好横跨冷暖两端，两种底色都托得住，**同一个控件不会换个主题就换性格**
- **邮箱哨兵的任务级模型**：`mailbox_sentinels` 新增 `provider` / `modelId` / `thinkingLevel` 三列（此前**根本没有模型字段**），编辑器里一行「模型」即可给这个 Inbox 单独钉一个模型与档位
- **markdown 图片块**：`![](…)` 从"语法原样显示"变成真的图 —— 圆角卡 + 文件名 / 尺寸行 + 点击走大图 sheet。支持本地路径 / `file://` / `~/` / 尖括号含空格路径，**相对路径按会话项目目录解析**；外链只给说明行不抓取（失败态同款外形、原字符串可选中）
- **表格列对齐 + 宽表横滚**：`|:---:|` / `|---:|` 三态生效（数字列右对齐）；列宽按内容收、超宽由外层横滚承担
- **代码块折行开关**：头部一颗 `↵ 换行` 药丸，**默认关 = 原横滚行为**；折行状态是块内局部态，不落库、不跨会话
- **工具产出的图片上屏**：工具结果 `content` 里 `type: "image"` 的块落盘后进工具卡「输出」区 —— 横排缩略条 + 点开大图 + 读不到照实说明（只有图片时也渲染该区）
- **诊断脚本三件**：`md_preview.py`（markdown 层出图 + 铺满列宽的机器量宽）· `toolcard_preview.py`（工具卡三态与调色板）· `db_upgrade_probe.py`（真库副本上的升级迁移 + 旧载荷回放）

### Changed

- **模型链路简化（四刀中的三刀）**：**刀 1** 撤除 P5.1 自定义模型层（判据 = 生产 UI 零调用点；`custom_models` 表与 `migrateLegacyCustomModels()` **有意保留** —— 已发版，旧库还得能读）· **刀 2** `ThinkingLevel.supported` 立为唯一实现 · **刀 4** 删掉仅供笛卡尔积 `ForEach` 使用的 `ModelMenuEntry` 系列，控件解耦成独立文件。**刀 3（删 `availableModels`）暂缓待拍板**
- **模型 + 级别两颗药丸合成一颗**，设置与配置页的模型选择 UI 随之简化
- **`ModelPicker` 接进 Schedule**（cron / watch / inbox 三种配置）：语义严格保持「**仅本任务、绝不写全局**」—— `onPick` 只写草稿，不碰当前会话的模型。**模式选择保持独立**（与模型链路正交，不合并）
- **编辑区列宽 760 → 1080**，并按"值相同也拆"的原则拆成三个 token（Schedule / 扩展 / 知识条目）—— 三处当下同值，但**不是同一条约束**（表单 / 清单 + 源码预览 / 长文编辑器）
- **Inbox 规则卡版式重做**：控件从系统 `.menu` / `.switch`（自带陌生蓝）换回自家一套，字段行按"**控件有无本征宽度**"分两种形状，且所有控件共用一条左边缘
- 「工具产图暂不上屏」的已知限制**已随本版删除**（本版实现）

### Fixed

- **未知工具被谎报为 READ**：`kindFor` 的 `default` 直接返回 `.read` ⇒ 装了 web-search / subagents 之类的扩展后，那些工具在卡片上会显示成 READ，**且已落库、事后分不出**。现新增中性档 `.other`，并把陌生 `rawValue` 在解码时降级（不让一条陌生 kind 把整条载荷解码搞崩）
- **工具输出整段丢失**：旧提取器只在顶层找字符串键，而契约是 `content[].text` ⇒ 连纯文本也丢（落库实测每张卡 `"details":[]`，失败的只剩一句「工具执行失败」）。现按契约重写：`content[].text` 合并 / 图片落盘挂进工具卡 / 无法识别时用 `details` 折平兜底 / **失败取真错误文本**
- **思考档位的两份实现已经漂移**：`ManagedModel` 是白名单（只收非空项）、pi 是黑名单（默认支持、显式 `null` 才剔除）⇒ 遇到 `thinkingLevelMap` 缺键的模型，档位会**少一档且不报错**。现 `ThinkingLevel.supported` 为唯一实现，并把那条**把 bug 当期望值**的断言改成守边界
- **「胶囊显示 xhigh、实际跑 off」**：切换模型时级别传 `nil`（靠 pi 侧兜底收敛），于是显示与实际执行割裂。现收敛到 `clampLevel` 一处
- 修正 `kindFor` 旁边那条注释的**位置错误**：工具卡的标签色由**相态**决定（与 kind 无关），只有轨迹页 chip 才吃 kind 色
- **一条 tooltip 的英文一直回落中文**：它已进译文真源但没有跑 `--write` ⇒ `.strings` 里从未生成。

### 工程

- **门禁水位**：冒烟 **741 → 797 项**；词表 **589 → 593**；e2e 指纹 **614 项**含 1 条反例。新增覆盖面：markdown 层（此前**零覆盖** —— 给 `table` 加关联值本来无人能红）、工具结果面（此前**零断言**）、模型选择值逻辑、哨兵任务级模型
- **八条新纪律**（都写进代码注释，各有实测依据）：① **撤下输入框 ≠ 清空数据**（旧条目编辑时只覆盖编辑器真能改的维）② **显示口径可以代理，存储口径不许被显示口径污染**（"未指定"控件表达不了，就"显示值代理、状态里保留 nil"）③ **删掉一条路径时，要问它顺带承担了什么语义**（菜单笛卡尔积一删，"档位兜底"必须落到 `thinkingStops`）④ **裁渐变只能用 `mask` 不能用窄 `frame`**（`LinearGradient` 按自身 frame 算比例，一缩就把整条渐变压进一小截）⑤ **「A 太窄 / B 太宽」先量两边底色，别先调宽度**（真因常是"一个有底、一个是裸标签"，或弹性控件吃光了余量）⑥ **"推右"治不了右侧留白** —— 它用"控件列的左边缘"换来，代价更大 ⑦ **落盘类夹具要重定向必须进程级**（生产调用点不传 `baseDirectory:` ⇒ 函数级注入管不到，夹具会写进用户真实目录而**零红灯**）⑧ **判据的容器参数必须与 App 同源**（手填的容器宽会让判据**自证** —— 探针里手填 620/16 出的 PASS 是在错误的列宽下拿到的，现改为直取 `Tune.*`，并让探针把期望值打印给判据脚本）
- **两处判据自己就是错的，已修**（"门绿着"的缺陷只能给**判据**造反例才抓得到）：接线门的分类表里有一条**万能匹配** ⇒ 把所有未分类可疑项都标成"位于多行字符串内"并给出**方向相反**的修法建议（**分类表里不许有万能匹配**）；`md_preview` 的量宽判据把背景判成"最高频色" ⇒ 表格铺满时它自己成了最高频，量出 620 而非 588（改取**角上那一像素**）。`md_preview.py --counterexample` 现在能一键造反例自证判据会红
- **离屏出图换渲染路径**：`ImageRenderer` **画不出 `ScrollView` 的内容**（高度对、内容空）⇒ 判据只能退成"比高度"。改用 `NSHostingView` + 离屏 `NSWindow` + `cacheDisplay`（跑真实 layout / display）后横滚内容照画，判据升级成**量宽度**。⚠️ 必须抽 RunLoop —— 铺满靠的是两趟布局，`onPreferenceChange` 要等一次 runloop 才送达，只 layout 一趟会误判成"没生效"
