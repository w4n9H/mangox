...
- 冒烟门禁：`scripts/smoke/run.sh` 一条命令跑 333 项语义冒烟（事件归并/并发路由/fire 后台化/上限拒绝/落库对拍/轨迹派生/settled 语义/状态栏数据链/过程态胶囊/Trace v2/侧问 fork/离开摘要/自动命名/模型物化/元数据目录/模式矩阵/附件管线/发送门控/日分组/审批阻塞/热键配置/捕获链路/备份引擎/重放缓存/无人值守 fire）
